...
int DouNextPrime(size_t n) {
  if (n % 2 == 0)
    ++n;  
  while (!DouIsPrime(n)) n += 2;  
  return n;
}
...
class HashTableDouble {
 public:
  ...
  explicit HashTableDouble(size_t size = 101) : array_(DouNextPrime(size))
    { MakeEmpty(); }
 ...
 private:        
  ...
  size_t FindPos(const HashedObj & x) const {
    ...
    while (array_[current_pos].info_ != EMPTY &&
     array_[current_pos].element_ != x) {
      current_pos += i * offset;  // Compute ith probe.
  ...
  void Rehash() {
    ...
    array_.resize(DouNextPrime(2 * old_array.size()));
    for (auto & entry : array_)
      entry.info_ = EMPTY;
